...
const awsReducer = (state = initialState, action) => {
  ...
  switch (action.type) {
    ...
    case types.ADD_INVOCATIONS_ALLDATA: {
      ...
      graphOptions.high = Math.max(Math.round(action.payload.options.maxInvocations / 100) * 100,100);
      ...
      series_data.unshift({x: moment(startTime).subtract(5,'minutes'), y: null})
