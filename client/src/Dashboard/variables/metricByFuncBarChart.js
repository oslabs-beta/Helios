...
const metricByFuncBarChart = (props, timePeriod, region) => {
  ...
  console.log('AWS Render By Func: ', props.awsByFunc.renderByFunc);
  console.log('Credentials: ', props.credentials);
  console.log(
    'LIST OF FUNCTIONS BY FUNC DATA BEING FETCHED FOR: ',
    props.aws.functions
  );
  ...
  fetch('/aws/getMetricsByFunc/Invocations', reqParams)
    ...
    .then((invocationData) => {
      ...
      console.log(
        'Printing from Inside By Func Invocations Bar Chart Func: ',
        props.invocationsByFuncData
      );
    })
